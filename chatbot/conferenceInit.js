...
function roomInit() {
  ...
  room.on(JitsiMeetJS.events.conference.CONFERENCE_FAILED, (error) => {
    ...
    if (
      lowerDetails.includes('membersonly') ||
      lowerDetails.includes('members-only')
    ) {
      log(
    ...
    if (failureDetails === JitsiMeetJS.errors.conference.CONFERENCE_DESTROYED) {
  ...
  room.setDisplayName(options.chatbotDisplayName)
...
function main() {
  ...
  document.title = 'Chatbot - ' + roomName
