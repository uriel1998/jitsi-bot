...
mapfile -t uniq_timestamps < <(printf '%s\n' "${timestamps[@]}" | sort -u)
if [[ ${#uniq_timestamps[@]} -gt 1 ]]; then
  echo "Multiple timestamps found: $(printf '%s\n' "${uniq_timestamps[@]}" | paste -sd ', ' -). Keep only one timestamp set in this directory." >&2
  exit 1
fi

timestamp="${uniq_timestamps[0]}"
